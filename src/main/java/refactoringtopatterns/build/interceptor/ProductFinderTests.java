...
public class ProductFinderTests  {
    ...
    public void testFindByColor() {
        ...
        assertEquals("found fireTruck", foundProducts.contains(fireTruck));
        assertEquals("found Toy Porsche Convertible", foundProducts.contains(toyConvertible));
    }
    ...
    public void testFindColorSizeAndBlowPrice() {
        List<Product> foundProducts = finder.byColorSizeAndBlowPrice(Color.red, ProductSize.SMALL.getSize(), 10.00f);
        assertEquals("found no small red product below $10.00", 0, foundProducts.size());

        foundProducts = finder.byColorSizeAndBlowPrice(Color.red, ProductSize.MEDIUM.getSize(), 10.00f);
